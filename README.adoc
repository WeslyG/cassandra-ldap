...
|group_role_mappings
|YAML list of LDAP group to Cassandra role mappings. A user can log in only if they match at least one configured LDAP group. The plugin grants missing Cassandra roles and revokes stale managed roles during sync.
