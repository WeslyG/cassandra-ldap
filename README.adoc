...
The configuration is driven by a YAML file and system properties which you need to start Cassandra with
to point that plugin to a configuration file.

The system property is `-Dcassandra.ldap.config.file=/path/to/ldap.yaml`. If
not set, it will try to resolve `$CASSANDRA_CONF/ldap.yaml`.
...
|group_role_mappings
|YAML list of LDAP group to Cassandra targets. `cassandra_roles` may contain role names and/or keyspace permission objects with `grant` and `keyspace`. A user can log in only if they match at least one configured LDAP group. The plugin grants missing Cassandra roles and keyspace permissions and revokes stale managed entries during sync.
...
The `ldap.yaml` file in the `conf` directory does not need to be changed, and with the above `docker run` it will work out of the box. You just
have to put it in `$CASSANDRA_CONF` or set respective configuration property as described above.

## Explanation of ldap_filter_template property

`ldap_filter_template` property is by default `(cn=%s)` where `%s` will be replaced by name you want to log in with.
...
authentication using a specified service account. This service account should be configured in the `ldap.yaml`
file using the `ldap_service_dn` and `ldap_service_password` properties. If `ldap_service_dn` is set, such a role will be created in database,
when not already present, upon node's start.

`ldap_service_dn` account, which will be automatically created, will be superuser in Cassandra.
...
authenticate against database first and if not successful against LDAP using filter `ldap_filter_template` which defaults to `(cn=%s)`
...
## LDAP Group To Cassandra Role Sync

LDAP access is controlled by `group_role_mappings`. A user must match at least one configured LDAP group to log in.
For every successful LDAP authentication, the plugin calculates the union of Cassandra roles mapped from the user's LDAP
groups and synchronizes the user's managed Cassandra role memberships by granting missing roles and revoking stale ones.

Keyspace permissions can be managed in the same mapping block:
...
This produces Cassandra grants equivalent to:

```sql
GRANT SELECT ON KEYSPACE system TO 'cn=bill,dc=example,dc=org';
GRANT ALL ON KEYSPACE testkeyspace_1 TO 'cn=bill,dc=example,dc=org';
```

If such managed permission disappears from the mapping, the plugin will revoke it on the next synchronized authentication.
